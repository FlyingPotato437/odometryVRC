...
public:
    ...
    void update(float measurement_x, float measurement_y, float measurement_theta) {
        ...
        float detS = S[0][0] * (S[1][1] * S[2][2] - S[2][1] * S[1][2]) -
...
void updateOdometry() {
    ...
    float targetPoints[][3] = { {100, 100, 3.141592653589793238/2}, {200, 100, 0}, {200, 200, 3.141592653589793238} };
...
void autonomous() {
    KalmanFilter2D filter(0, 0, 0); // Initialize with the starting position and orientation

    // Define target points and follow the path using chained movement
    float targetPoints[][3] = { {100, 100, 3.14159265358979323846264338/2}, {200, 100, 0}, {200, 200, 3.14159265358979323846264338} };
    int numPoints = sizeof(targetPoints) / sizeof(targetPoints[0]);
